...
public class HQLGenerator {
    ...
    private static final String GETCONTENT_ACCOUNTS_HQL = "from TalkContent obj where ((obj.srcAccount = '"
	    + VALUE_PLACER + "' and targetAccount = '" + VALUE_PLACER + "')  or (obj.srcAccount = '" + VALUE_PLACER
	    + "' and targetAccount = '" + VALUE_PLACER + "'))";
    
    private static final String GETCONTENT_ACCOUNTS_TIME_HQL = GETCONTENT_ACCOUNTS_HQL + " and time >= '"
    ...
    public static String generateGetContentByAccounts(Account account, Account otherAccount) {
	List<String> values = new ArrayList<String>();
	values.add(String.valueOf(account.getId()));
	values.add(String.valueOf(otherAccount.getId()));
	values.add(String.valueOf(otherAccount.getId()));
	values.add(String.valueOf(account.getId()));
	String hql = GETCONTENT_ACCOUNTS_HQL;
    ...
    public static String generateGetContentByAccountAndTime(Account account, Account otherAccount, TimeInterval timeInterval){
	List<String> values = new ArrayList<String>();
	values.add(String.valueOf(account.getId()));
	values.add(String.valueOf(otherAccount.getId()));
	values.add(String.valueOf(otherAccount.getId()));
	...
	String hql = GETCONTENT_ACCOUNTS_TIME_HQL;
	for (int i = 0; i < values.size(); i++) {
	    hql = hql.replaceFirst(VALUE_PLACER, values.get(i));
	}
	System.out.println(hql);
